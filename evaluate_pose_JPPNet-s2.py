...
NUM_STEPS = 20  # Number of images in the validation set.
INPUT_SIZE = (384, 384)
N_CLASSES = 20
...
def main():
    ...
    image_batch125 = tf.image.resize(
        image_batch_origin, [int(h * 1.25), int(w * 1.25)])
    image_batch075 = tf.image.resize(
    ...
    with tf.compat.v1.variable_scope('', reuse=True):
        ...
        pose_out3_125, pose_fea3_125 = pose_refine(
            pose_out2_125, parsing_out2_125, pose_fea2_125, name='fc3_pose')
    ...
    pose_out3 = tf.reduce_mean(input_tensor=tf.stack([tf.image.resize(pose_out3_100, tf.shape(input=image_batch_origin)[1:3, ], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
                                                      tf.image.resize(pose_out3_125, tf.shape(input=image_batch_origin)[
                                                                      1:3, ], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
    ...
    output_all = tf.reduce_mean(input_tensor=tf.stack(
    ...
    with open('{}/{}.txt'.format(out_dir, img_id), 'w') as f:
        for p in range(image.shape[2]):
            ...
            if channel_.shape[0] != rows or channel_.shape[1] != cols:
                ...
                channel_ = imageio.imresize(
                    channel_, [rows, cols], interp='nearest')
