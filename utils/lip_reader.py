...
def image_mirroring(img, label, label_rev, heatmap, heatmap_rev):
    ...
    label_ = tf.boolean_mask(tensor=label_and_rev, mask=mask)
    label_ = tf.reshape(label_, tf.shape(input=label))

    heatmap_and_rev = tf.stack([heatmap, heatmap_rev])
    heatmap_ = tf.boolean_mask(tensor=heatmap_and_rev, mask=mask)
    heatmap_ = tf.reshape(heatmap_, tf.shape(input=heatmap))
...
class LIPReader(object):
    ...
    def __init__(self, data_dir, data_list, data_id_list, input_size, random_scale,
        ...
        self.labels_rev = tf.convert_to_tensor(value=self.label_rev_list, dtype=tf.string)
        self.poses = tf.convert_to_tensor(value=self.pose_list, dtype=tf.string)
